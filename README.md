...
## 🚀 Features
- **User Authentication**: Secure JWT-based authentication (Access & Refresh tokens) using HTTP-only cookies. Includes strict email verification and a cryptographically secure "Forgot Password" flow powered by the Resend SDK.
...
- **Email Dispatch**: Resend Node.js SDK (Asynchronous dispatch)
...
│   │   └── utils/          # Cloudinary config, Email dispatch, API responses
...
SMTP_FROM_EMAIL="Lumina Support" <support@your-domain.com>
...
- **Asynchronous Emails**: Email dispatch for OTPs and password resets runs completely in the background, ensuring instant API responses for the frontend.
